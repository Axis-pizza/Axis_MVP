            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {loading ? (
                ...
                filteredTokens.slice(0, 50).map(token => (
                  ...
                  >
                    {token.logoURI ? <img src={token.logoURI} className="w-8 h-8 rounded-full" /> : <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-xs font-bold">{token.symbol[0]}</div>}
                    <div>
                ...
                <div className="text-center py-12 text-white/30">No tokens found</div>
              )}
            </div>
          </motion.div>
        </>
