...
export const StockTokenCard = memo(({
  ...
  return (
    ...
    >
      {/* Selection Badge */}
      <div className={`absolute top-3 right-3 w-6 h-6 rounded-full flex items-center justify-center border transition-colors ${
        isSelected
      ...
      <div className="text-center w-full">
        <div className={`text-lg font-bold truncate mb-0.5 ${isSelected ? 'text-amber-400' : 'text-white'}`}>
          {token.symbol}
        </div>
        <div className="text-[10px] text-white/40 truncate w-full px-1">
          {token.name}
        </div>
      </div>
