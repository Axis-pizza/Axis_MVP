...
export const BacktestChart = ({ 
  ...
  if (!isValid) {
    return (
      <View className="flex items-center justify-center text-white/20 text-xs" style={{ height }}>
  ...
  return (
    <View className="relative w-full">
      ...
      <View className="flex items-center justify-between mb-4">
        <View>
          <Text className="text-xs text-white/40 mb-1 font-bold uppercase tracking-wider">{label}</Text>
          <Text className={`text-2xl font-mono font-bold ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
            {isPositive ? '+' : ''}{change.toFixed(2)}%
          </Text>
        </View>
        {showMetrics && (
          <View className="flex gap-3 text-right">
            {data.sharpeRatio !== undefined && (
              <View>
                <Text className="text-[10px] text-white/40 uppercase">Sharpe</Text>
                <Text className="text-xs font-bold text-white/80">{data.sharpeRatio.toFixed(2)}</Text>
              </View>
            )}
            {data.maxDrawdown !== undefined && (
              <View>
                <Text className="text-[10px] text-white/40 uppercase">Max DD</Text>
                <Text className="text-xs font-bold text-red-400">{data.maxDrawdown.toFixed(1)}%</Text>
              </View>
            )}
          </View>
        )}
