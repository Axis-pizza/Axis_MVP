      ...
      {isOpen && (
        <>
          ...
          >
            ...
            <View className="max-w-md mx-auto relative font-serif">
              ...
              <motion.div 
                ...
                className="mb-8"
              >
                <Text className="text-center text-xs text-white/30 font-sans mb-3 tracking-widest uppercase">Direct Line</Text>
                ...
                >
                  <View className="absolute inset-0 bg-gradient-to-br from-orange-950/80 via-black to-neutral-950 opacity-90 transition-opacity group-hover:opacity-100" />
                  <View className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20 mix-blend-overlay" />

                  <View className="relative p-5 flex items-center gap-4">
                   
                    <View className="relative">
                      <View className="absolute inset-0 bg-orange-500 blur-md opacity-20 rounded-full" />
                      <View className="relative w-14 h-14 rounded-full overflow-hidden border-2 border-orange-500/30">
                        <img 
                          src="/muse.png" 
                          alt="Founder" 
                    ...
                    <View className="flex-1 min-w-0">
                      <View className="flex items-center gap-2 mb-1">
                        <Text className="text-white font-serif text-xl italic tracking-wide">Muse</Text>
                        <Text className="px-2 py-0.5 bg-orange-500/10 border border-orange-500/20 text-orange-400 text-[9px] font-bold rounded-sm tracking-widest">FOUNDER</Text>
                      </View>
                      <View className="flex items-center gap-1.5 text-white/40 text-xs font-sans">
                        <Send size={12} className="text-orange-500/50" />
                        <Text className="group-hover:text-white/70 transition-colors">@muse_jp</Text>
                      </View>
                    </View>

                    
                    <View className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white/20 border border-white/5 group-hover:bg-orange-500 group-hover:text-black group-hover:border-orange-500 transition-all duration-300">
              ...
              <View className="flex items-center gap-4 mb-8">
                <View className="h-px bg-gradient-to-r from-transparent via-white/10 to-transparent w-full" />
                <Text className="text-[10px] text-white/20 whitespace-nowrap font-sans tracking-widest">OR SEND SIGNAL</Text>
                <View className="h-px bg-gradient-to-r from-transparent via-white/10 to-transparent w-full" />
              </View>
              ...
              {status === 'success' ? (
                ...
                >
                  <View className="w-16 h-16 bg-orange-500/10 rounded-full flex items-center justify-center mx-auto mb-4 text-orange-400 border border-orange-500/20">
                    <Send size={32} />
                  </View>
                  <Text className="text-2xl italic text-white mb-2">Signal Transmitted.</Text>
                  <Text className="text-sm font-sans text-white/40">We will analyze your report shortly.</Text>
                </motion.div>
              ) : (
                <form onSubmit={submitReport} className="space-y-5">
                  ...
                  <View className="space-y-1.5">
                    ...
                    <View className="relative group">
                      <Text className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-orange-500 transition-colors font-mono">@</Text>
                      <input 
                        name="user_tg"
                        type="text" 
                        value={tgId}
                  ...
                  <View>
                    ...
                    {!selectedFile ? (
                      ...
                      >
                        ...
                        <Text className="text-xs font-sans tracking-wide">Attach Screenshot</Text>
                      </motion.button>
                    ) : (
                      <motion.div 
                        initial={{ opacity: 0, y: 10 }}
                      ...
                      >
                        ...
                        {previewUrl && (
                          <View className="h-32 w-full bg-[#111] relative">
                             <img src={previewUrl} alt="Preview" className="w-full h-full object-cover opacity-60" />
                          </View>
                        )}
                        

                        <View className="absolute inset-0 flex items-center justify-between p-4 bg-gradient-to-t from-black/80 to-transparent">
                           <Text className="text-xs text-white font-mono truncate max-w-[70%]">{selectedFile.name}</Text>
                           <button 
                             type="button"
                             onPress={clearFile}
