...
export const ProfileEditModal = ({
  ...
  return createPortal(
    <AnimatePresence>
      <View className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100000] flex items-center justify-center p-4">
        <motion.div
        ...
        >
          <View className="p-6 border-b border-white/5 flex items-center justify-between">
            <h2 className="text-xl font-bold text-white">
              {isExistingUser ? 'Edit Profile' : 'Create Account'}
            </h2>
            <button onPress={onClose} className="p-2 hover:bg-white/10 rounded-full text-white/70">
              <X className="w-5 h-5" />
            </button>
          </View>
            ...
            <View className="space-y-6">
              <View className="flex flex-col items-center">
                ...
                >
                  ...
                  <View className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                    <UploadCloud className="w-8 h-8 text-white" />
                  </View>
                </View>
                <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*" className="hidden" />
                <Text className="text-xs text-white/30 mt-2">Tap to upload</Text>
              </View>
