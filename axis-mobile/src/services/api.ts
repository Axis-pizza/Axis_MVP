/**
 * API Service - Centralized API calls (React Native version)
 */
...
export const api = {
  getUser: async (pubkey: string) => {
    try {
      const ref = await AsyncStorage.getItem('axis_referrer');
  ...
  async uploadProfileImage(imageUri: string, walletAddress: string) {
  ...
  async uploadImage(imageUri: string, walletAddress: string, type: 'strategy' | 'profile' = 'strategy') {
    const formData = new FormData();
    formData.append('image', {
      uri: imageUri,
      type: 'image/jpeg',
