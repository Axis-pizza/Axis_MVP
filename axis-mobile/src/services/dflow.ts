...
function buildToken(event: EventDef, side: 'YES' | 'NO'): JupiterToken {
...
const XSTOCK_SYMBOLS = [
  "AAPLx", "TSLAx", "NVDAx", "MSFTx",
  "AMZNx", "GOOGLx", "SPYx", "QQQx",
];
...
export async function fetchStockTokens(): Promise<JupiterToken[]> {
  ...
  return found.map(t => ({ ...t, price: prices[t.address] ?? t.price }));
}
...
export async function fetchCommodityTokens(): Promise<JupiterToken[]> {
  try {
    ...
    return tokens.map(t => ({ ...t, price: prices[t.address] ?? t.price }));
  } catch (e) {
